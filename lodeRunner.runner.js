...
function moveRunner() {
  ...
  if (curToken == LADDR_T || (curToken == BAR_T && yOffset == 0)) {
    //ladder & bar
  ...
  switch (keyAction) {
    ...
    case ACT_DOWN:
      ...
      if (!(yOffset >= 0 && stayCurrPos)) moveStep = ACT_DOWN;
      break;
    case ACT_LEFT:
      stayCurrPos =
        x <= 0 ||
      ...
      if (!(xOffset <= 0 && stayCurrPos)) moveStep = ACT_LEFT;
      break;
    case ACT_RIGHT:
      stayCurrPos =
        x >= maxTileX ||
...
function runnerMoveStep(action, stayCurrPos) {
  ...
  if (action == ACT_DOWN || action == ACT_FALL) {
    ...
    if (curToken == BAR_T) {
      if (yOffset < 0) holdOnBar = 1;
        ...
        if (
          action == ACT_DOWN &&
          y < maxTileY &&
          map[x][y + 1].act != LADDR_T &&
          map[x][y + 1].act != GUARD_T
        ) {
          action = ACT_FALL;
        }
      }
    ...
    if (action == ACT_DOWN) {
      ...
      if (y < maxTileY && map[x][y + 1].act == GUARD_T) {
        //over guard
        //don't collision
...
function decGold() {
  if (--goldCount <= 0) {
    ...
    if (runner.pos.y > 0) {
      if (curTheme == 'C64')
        soundPlay('goldFinish' + (((curLevel - 1) % 6) + 1)); //six sounds
  ...
  switch (nextMove) {
    case ACT_DIG_LEFT:
      ...
      if (
        y < maxTileY &&
        x > 0 &&
        map[x - 1][y + 1].act == BLOCK_T &&
        map[x - 1][y].act == EMPTY_T &&
        map[x - 1][y].base != GOLD_T
      )
        rc = 1;
      break;
    case ACT_DIG_RIGHT:
...
function isDigging() {
  ...
  if (holeObj.action == ACT_DIGGING) {
    var x = holeObj.pos.x,
      y = holeObj.pos.y;
    if (map[x][y].act == GUARD_T) {
      //guard come close to the digging hole !
      var id = getGuardId(x, y);
      if (
        holeObj.sprite.currentAnimationFrame < holeObj.digLimit &&
        guard[id].pos.yOffset > -H4
      ) {
        if (DEBUG_DIG)
          loadingTxt.text =
            'dig : ' + holeObj.sprite.currentAnimationFrame + ' (X)';

        stopDigging(x, y);
      } else {
        if (DEBUG_DIG)
          loadingTxt.text =
            'dig : ' + holeObj.sprite.currentAnimationFrame + ' (O)';
        if (curAiVersion >= 3) {
...
function fillHole(x, y) {
  ...
  if (curAiVersion < 3) {
    fillSprite.on('animationend', fillComplete, null, false, {
      obj: fillSprite,
    });
    fillSprite.play();
  } else {
...
function fillComplete(evt, data) {
  ...
  switch (map[x][y].act) {
    ...
    case GUARD_T: //guard dead
      var id = getGuardId(x, y);
      if (curAiVersion >= 3 && guard[id].action == ACT_IN_HOLE)
        removeFromShake(id);
  ...
  if (DEBUG_TIME)
    loadingTxt.text = 'FillHoleTime = ' + (recordCount - fillHoleTimeStart); //for debug
}
